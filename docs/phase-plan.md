...
## 今後の方向性メモ: 統一チャット（4アプリ → 1チャット）

現状はユーザー向け入口が 4 アプリ（質問 / 登録 / 一括取り込み / 重複排除）に分かれている。
これを **1 つのチャット入口**に統合し、裏で各機能をスキル/ツールとして呼び分ける方向へ
進める（ユーザーはどの Bot を開くか迷わなくてよい）。

- v1 は**決定的ルーティング**（キーワード・添付有無等での振り分け）から始め、
  エージェント的な自律判断は後段で検討する。
- 各機能の中身（Adapter 契約・決定的コア・HITL の承認モデル）はこのまま流用する。
  統合されるのは「入口」であり、疎結合の設計原則（design-principles.md）は変えない。
